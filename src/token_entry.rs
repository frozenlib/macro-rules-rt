...
/// Maps token entries to their corresponding byte offsets in the source text.
///
/// Each element in the vector represents a token entry's position:
/// - `Some(offset)`: The token entry starts at the given byte offset in the source text
/// - `None`: The token entry has no corresponding position in the source text
///   (e.g., tokens generated by macro expansion)
///
/// This mapping is essential for correlating parsed tokens with their original
/// source code locations, especially when dealing with macro-generated code.
#[derive(Debug)]
...
impl<'a> Source<'a> {
    ...
    /// Retrieves the source text and token entries for a given range of token entries.
    ///
    /// This method splits the specified token entry range into three parts:
    /// 1. Token entries at the beginning that have no corresponding source text
    /// 2. The actual source text substring that corresponds to the mappable tokens
    /// 3. Token entries at the end that have no corresponding source text
    ///
    /// # Arguments
    /// * `tes_range` - The range of token entries to retrieve
    ///
    /// # Returns
    /// A tuple containing:
    /// - `&[TokenEntry]` - Leading token entries with no source correspondence
    /// - `&str` - The source text substring for mappable tokens
    /// - `&[TokenEntry]` - Trailing token entries with no source correspondence
    ///
    /// If no tokens in the range have source correspondence, the middle string will be empty.
